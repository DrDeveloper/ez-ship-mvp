    ...
    <main class="content">
        ...
        try {
            $parcelStmt = $db->prepare("SELECT p.pid, p.ps, p.pd, p.pv, p.pt, pr.wid, w.wl, pr.rid, pr.recipient_type,
                    CASE
                        WHEN pt.prt IS NOT NULL THEN CONCAT('Delivered at: ', pt.prt)
                        WHEN pl.pl = c.cid THEN CONCAT('In Client''s Possession: ', c.cn)
                        WHEN pl.pl = w2.wid THEN CONCAT('Stored At Warehouse: ', w2.wn)
                        WHEN pl.pl = d.did THEN CONCAT('Driver, ', d.dn, ' En Route')
